...
class LMCacheKVCacheContext:
    ...
    def __init__(
        ...
        try:
            ...
            self._non_cuda_context_meta_conf = response.json().get(
    ...
    def _register_from_non_cuda_entry(self, entry: dict) -> None:
    ...
    def register_kv_caches(
        ...
        # check nom-gpu
        if not entry:
            for e in self._non_cuda_context_meta_conf.values():
                if e.get("model_name") == self._model_name:
                    entry = e
                    gpu_entry = False
                    break
        ...
        if not entry:
            raise KVCacheSDKError(
                f"no registered GPU and non-GPU layout for model_name={self._model_name!r}; "
        ...
        if gpu_entry:
            ...
            block_size, head_dim = self._register_from_non_cuda_entry(entry=entry)
            num_kv_heads = 1
