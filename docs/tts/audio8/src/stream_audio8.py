...
def _crossfade(prev: np.ndarray, nxt: np.ndarray, n: int) -> np.ndarray:
    """Blend the first *n* samples of *nxt* with the tail of *prev*."""
    if n <= 0 or prev.size == 0 or nxt.size == 0:
        return nxt
    n = min(n, prev.size, nxt.size)
    out = nxt.copy()
    fade_out = np.linspace(1.0, 0.0, n, dtype=np.float32)
    fade_in = np.linspace(0.0, 1.0, n, dtype=np.float32)
    out[:n] = prev[-n:] * fade_out + nxt[:n] * fade_in
    return out


class _PlaybackBuffer:
    """Accumulates audio and plays from a background thread after prefetch."""

    def __init__(self, sample_rate: int, prefetch_seconds: float = 1.0):
        import queue
        import threading
    ...
    def feed(self, chunk: np.ndarray) -> None:
        if self._closed or chunk.size == 0:
            return
        self._queue.put(np.clip(chunk, -1.0, 1.0).astype(np.float32, copy=False))
...
class _CodeStreamer:
    """Collects generated code tokens, decodes in chunks, and streams playback."""
    ...
    def __init__(
        ...
        *,
        ...
        self._crossfade_samples = max(0, int(sample_rate * crossfade_ms / 1000.0))

        self.frames: list[torch.Tensor] = []
        self.emitted_frames = 0
        self._prev_tail: np.ndarray | None = None
        self._playback = _PlaybackBuffer(sample_rate, prefetch_seconds) if play else None

    def put(self, value: torch.Tensor) -> None:
        if self.debug and len(self.frames) == 0:
            print(f"[stream] first put() value shape={tuple(value.shape)} dtype={value.dtype}")
        step = value.detach().to("cpu").reshape(-1)
        if step.numel() < self.num_codebooks:
            return
        self.frames.append(step[: self.num_codebooks])

        if len(self.frames) % self.chunk_frames != 0:
            return
        self._decode_and_emit()

    def _decode_and_emit(self) -> None:
        total_frames = len(self.frames)
        if total_frames <= self.emitted_frames:
            return

        start_frame = max(0, self.emitted_frames - self.overlap_frames)
        window = torch.stack(self.frames[start_frame:total_frames], dim=1).unsqueeze(0)
        with torch.inference_mode():
            waveforms, waveform_lengths = self.model.decode_audio(window.to(self.model.device))
        audio = waveforms[0, : int(waveform_lengths[0])].float().cpu().numpy()

        spf = self.samples_per_frame
        slice_start = (self.emitted_frames - start_frame) * spf
        new_frames = total_frames - self.emitted_frames
        slice_end = min(slice_start + new_frames * spf, audio.size)
        if slice_start >= slice_end:
            return

        chunk = np.ascontiguousarray(audio[slice_start:slice_end])
        if self._prev_tail is not None:
            chunk = _crossfade(self._prev_tail, chunk, self._crossfade_samples)
        if self._crossfade_samples > 0 and chunk.size > 0:
            self._prev_tail = chunk[-self._crossfade_samples :].copy()
        else:
            self._prev_tail = None

        self.emitted_frames = total_frames
        if self._playback is not None:
            self._playback.feed(chunk)
    ...
    def full_codes(self) -> np.ndarray:
        if not self.frames:
            return np.zeros((self.num_codebooks, 0), dtype=np.int64)
        return torch.stack(self.frames, dim=1).numpy().astype(np.int64)
